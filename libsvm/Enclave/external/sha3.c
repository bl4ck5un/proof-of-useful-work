...
void
sha3_Update(sha3_context *ctx, uint8_t const *bufIn, size_t len) {
 ...
 */
void sha3_Finalize(sha3_context *ctx, uint8_t* hash) {
    ...
    memcpy(hash, ctx->sb, ctx->capacityWords / 2 * sizeof(uint64_t));
}