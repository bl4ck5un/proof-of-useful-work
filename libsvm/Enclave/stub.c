...
int run(pow_spec *spec, difficulty_t* difficulty, block_hash_t *cur_hash,
        sgx_target_info_t *qe_info, sgx_report_t *report, output_t *output)
{
    sgx_status_t ret;

    long n_work = pouw_main(spec, output);

    LL_DEBUG("n_work is %ld", n_work);
